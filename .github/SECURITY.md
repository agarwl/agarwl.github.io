...
The site’s own HTML must retain its restrictive Content Security Policy, use only local CSS, images, and fonts, and must not load JavaScript, tracking pixels, remotely hosted fonts, or embedded players. Link to videos and interactive external resources instead. CI enforces this baseline; the policy does not protect separate project repositories or external links after navigation.
